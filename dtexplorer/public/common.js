...
window.do58Encode = function (instr) {
    if (instr.length < 2 || (instr.length & 1) != 0) {
        return;
    }
    var sbytes = hexStr2byteArray(instr);
    let address = getBase58CheckAddress(sbytes);
    return address;
}

window.do58Decode = function (address) {
    var bytes = decodeBase58Address(address);
    let shex = byteArray2hexStr(bytes);
    return shex;
}
    ...
    function x(c) {
        ...
        if (0 === c.lastIndexOf("SHA-", 0)) {
            switch (a = [3238371032, 914150663,
                ...
                3204075428
            ], d = [1779033703, 3144134277, 1013904242, 2773480762,
